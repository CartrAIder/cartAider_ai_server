...
1. 결제 완료 후 발급된 `gate_token`과 설치된 게이트의 `gate_id`를 확보한다.
2. 좌·우 카메라에서 같은 카트를 촬영한 연속 프레임을 준비한다.
3. 두 카메라의 프레임을 하나의 HTTP multipart 요청으로 AI 서버에 전송한다.
4. AI 서버가 반환한 `PASS`, `REVIEW`, `FLAG`를 게이트 제어 로직에 전달한다.

AI 서버가 직접 카메라를 제어하거나 영상을 스트리밍으로 가져가지는 않는다. 게이트 장치가 캡처를 끝낸 이미지 묶음을 업로드한다.

## 2. 전체 처리 흐름

```text
결제 완료 및 gate_token 수신
  → 카트가 촬영 위치에 진입
  → cam_left / cam_right 연속 프레임 캡처
  → AI 서버에 multipart 업로드
  → AI 서버가 Spring에서 결제 상품 목록 조회
  → 상품 검출·추적·인식
  → 좌·우 카메라의 같은 물체 융합
  → 결제 상품 및 수량과 비교
  → AI 서버가 Spring에 최종 결과 저장
